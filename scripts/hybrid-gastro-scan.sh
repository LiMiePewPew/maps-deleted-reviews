...
NOTICE_WORKERS="${NOTICE_WORKERS:-1}"
...
if [[ -n "$GOSOM_GRID_BBOX" ]]; then
  ...
  gosom_args+=( -grid-bbox "$GOSOM_GRID_BBOX" -grid-cell "$GOSOM_GRID_CELL" -zoom "$GOSOM_ZOOM" )
fi
"$GOSOM_BIN" "${gosom_args[@]}"

echo
echo "== Stage 2: certified Pipeline V2 notice check =="
notice_mode=(--headed)
if [[ "$NOTICE_HEADLESS" == "1" ]]; then notice_mode=(--headless); fi

npm start -- \
  --city "$CITY" \
  --country "$COUNTRY" \
  --places-file "$DISCOVERY_FILE" \
  --workers "$NOTICE_WORKERS" \
  "${notice_mode[@]}"
...
echo "Resume state: output/state-${SAFE_CITY}-places-v2.json"
